...
<div class="container">
    ...
    <div class="row py-2">
        <form:form action="/san-pham/add" method="POST" modelAttribute="dongSp">
            <div class="mb-3">
    ...
    <div class="row py-2">
        <form class="d-flex" role="search" action="/san-pham/search" method="get">
    ...
    <table class="table table-striped table-hover">
        ...
        <c:forEach var="dongsp" items="${list.content}" varStatus="i">
            <tr>
                ...
                <td>${dongsp.ma}</td>
                <td>${dongsp.ten}</td>
                <td><a href="/san-pham/view-update?id=${dongsp.id}"
                       class="btn btn-outline-warning"><i
                        class="bi bi-pencil"></i> Edit</a></td>
                <td><a href="/san-pham/chi-tiet?id=${dongsp.id}"
                       class="btn btn-outline-warning"><i
                        class="bi bi-pencil"></i> Chi Tiết Sản Phẩm</a></td>
                <td><a href="/san-pham/remove?id=${dongsp.id}"
        ...
        <ul class="pagination  justify-content-center">
            <c:forEach begin="0" end="${list.totalPages - 1}" varStatus="i">
                <li class="page-item ${i.index == list.number ? 'active' : ''}">
                    <a class="page-link" href= "/san-pham/hien-thi?page=${i.index}&size=${listSp.size}">
                            ${i.index + 1}
                    </a>
                </li>
            </c:forEach>
