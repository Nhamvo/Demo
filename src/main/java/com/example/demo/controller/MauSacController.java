package com.example.demo.controller;

import com.example.demo.entity.ChiTietSp;
import com.example.demo.entity.MauSac;
import com.example.demo.entity.Nsx;
import com.example.demo.service.MauSacService;
import jakarta.validation.Valid;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.PageRequest;
import org.springframework.data.domain.Pageable;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.validation.BindingResult;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.ModelAttribute;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestParam;

import java.util.List;
import java.util.UUID;

@Controller
public class MauSacController {
    @Autowired
    private MauSacService service;

    @GetMapping("/mau-sac/hien-thi")
    public String hienThi(Model model, @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "4") int size) {
        Pageable pageable = PageRequest.of(page, size);
        Page<MauSac> cuaHangPage = service.getAll(pageable);
        model.addAttribute("list", cuaHangPage);
        model.addAttribute("dongSp", new MauSac());
        return "/mausac/viewMauSac";
    }
    @GetMapping("/mau-sac/remove")
    public String delete(@RequestParam("id") UUID id, Model model) {
        MauSac nv = service.findById(id);
        service.delete(nv);
        return "redirect:/mau-sac/hien-thi";
    }

    @PostMapping("/mau-sac/add")
    public String add(@ModelAttribute("dongSp") @Valid MauSac nsx, BindingResult result, Model model,
                      @RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "4") int size) {
        if (result.hasErrors()) {
            Pageable pageable = PageRequest.of(page, size);
            Page<MauSac> cuaHangPage = service.getAll(pageable);
            model.addAttribute("list", cuaHangPage);
            return "/mausac/viewMauSac";
        }
        service.save(nsx);
        return "redirect:/mau-sac/hien-thi";
    }

    @GetMapping("/mau-sac/view-update")
    public String edit(Model model, @RequestParam("id") UUID id) {
        MauSac nsx = service.findById(id);
        model.addAttribute("dongSp", nsx);
        return "/mausac/viewUpdate";
    }

    @PostMapping("/mau-sac/update")
    public String update(@ModelAttribute("dongSp") @Valid MauSac nsx, BindingResult result) {
        if (result.hasErrors()) {
            return "/mausac/viewUpdate";
        }
        service.save(nsx);
        return "redirect:/mau-sac/hien-thi";
    }

    @GetMapping("/mau-sac/search")
    public String search(
            @RequestParam("ten") String ten,
            Model model) {
        List<MauSac> listSearch = service.listSearch(ten, ten);

        if (ten.isBlank()) {
            model.addAttribute("thongBao", "không được để trống");
            model.addAttribute("dongSp", new MauSac());
            model.addAttribute("list", service.getAll());
            return "/mausac/viewMauSac";
        } else {
            if (listSearch.isEmpty()) {
                model.addAttribute("thongBao", "không tìm thấy");
                model.addAttribute("searchName", ten);
                model.addAttribute("dongSp", new MauSac());
                return "/mausac/viewMauSac";
            }
            else {
                model.addAttribute("dongSp", new MauSac());
                model.addAttribute("list", listSearch);
                model.addAttribute("searchName", ten);
                return "/mausac/viewMauSac";

            }
        }

    }
}
